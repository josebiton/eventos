    ...
    <div class="table table-responsive">
        <table class="table table-striped table-bordered table-hover inquilinos" >
            ...
            <tbody>
                ...
                $ListLogins = mysqli_query($link, "SELECT id_login,id_persona, id_sucursal FROM login");
                while ($listlogins = mysqli_fetch_array($ListLogins)):
                $idlogin = $listlogins['id_login'];
                $idsucursal = $listlogins['id_sucursal'];              
                $id_persona = $listlogins['id_persona'];              
                
                ?>
                <tr class="gradeX">
                    <td> <?php
                        $sucursal = mysqli_fetch_assoc(mysqli_query($link, "SELECT nombre FROM sucursales WHERE id_sucursal='$idsucursal'"));
                    ...
                    <td class="center">
                        <a href="javascript:;" onclick="AsignAccessM('<?php echo $id_persona; ?>', '<?php echo $idlogin; ?>', '<?php echo $idsucursal; ?>')" title="Asignar Acceso" ><span class="fa fa-arrow-right">&nbsp;&nbsp;&nbsp;</span></a>
